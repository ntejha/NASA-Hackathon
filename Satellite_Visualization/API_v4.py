...
def fetch_landsat_image(latitude, longitude, date):
    ...
    try:
        ...
        # Check if no imagery is available for the specified date
        if response.json().get("msg") == "No imagery for specified date.":
            print("No imagery available for the specified date.")
            return False
...
while True:
    ...
    # Fetch Landsat image
    if fetch_landsat_image(latitude, longitude, date):
        break
    else:
        print("Re-rolling...")
