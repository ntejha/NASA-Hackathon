...
def get_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--data', type=str, default='/home/akz-workhorse/programming/NASA-Hackathon/Torch_Segmentation/Semantic segmentation of aerial imagery/Semantic segmentation dataset', help='path to your dataset')
...
def acc(label, predicted):
    seg_acc = (label == torch.argmax(predicted, axis=1)).sum() / torch.numel(label)
    return seg_acc
